...
__all__ = ['models_close', 'fit_model', 'fit_adjust_model',
           'solve_partial_conditions', 'hit_boundary', 'dependence_hit_boundary',
           'display_model', 'get_model_loss', 'set_N_cpus', 'solve_all_conditions',
           'gddm']
...
def fit_adjust_model(sample, model, fitparams=None, fitting_method="differential_evolution",
    ...
    "differential_evolution" is the default, which accurately locates the global
    maximum without using a derivative.  For most purposes, differential
    evolution is the recommended method.  Another option is "bads" for BADS
    (https://arxiv.org/abs/1705.04405), which may be faster for some models.
    Other methods are only recommended in special cases.  "simple" uses a
    derivative-based method to minimize, and just uses randomly initialized
    parameters and gradient descent, and "simplex" is the Nelder-Mead method,
    and is a gradient-free local search.  These are only recommended if you have
    a very simple model, and also have a good estimate of the global
    maximum. Alternatively, a custom objective function may be used by setting
    `fitting_method` to be a function which accepts the "x_0" parameter (for
    starting position) and "constraints" (for min and max values).  In general,
    it is recommended you almost always use differential evolution or BADS,
    unless you have a model which is highly-constrained (e.g. only one or two
    parameters to estimate with low covariance) or you already know the
    approximate parameter values.  If you change this value, make sure to check
    that you get reliable and recoverable fits.
    ...
    elif fitting_method == "bads":
        if not HAS_BADS:
            raise SystemError("PyBADS is not installed, please install with \"pip install pybads\"")
        bounds_lower = [b[0] for b in constraints]
        bounds_upper = [b[1] for b in constraints]
        if not verbose:
            fitparams['display'] = "off"
        x_fit = pybads.BADS(_fit_model, x_0, bounds_lower, bounds_upper, bounds_lower, bounds_upper, options=fitparams)
        x_fit.optimize()
        x_fit.fun = x_fit.yval
        x_fit.message = x_fit.optim_state['termination_msg']
