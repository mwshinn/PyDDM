...
def DDM_pdf_general(params, setting_index, task_index=0):
    ...
    for i_t in range(len(t_list_temp)-1):
        ...
        if sum(pdf_list[:, i_t])>0.00001:                                                                               # For efficiency only do diffusion if there's at least some densities remaining in the channel.
            ...
            # # TEMP: 2 end points as absorbing state
            matrix_diffusion[0,0]   -= 1.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[0] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            matrix_diffusion[-1,-1] -= 1.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[-1] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)

            # matrix_diffusion[1,0]   += (0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[0] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            # matrix_diffusion[-2,-1] += (0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[-1] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            # matrix_diffusion[0,1]   += 0.5*dt/dx *(mu_temp + param_mu_x_temp*x_list[1] + param_mu_t_temp*t_list_temp[i_t] + param_B_t_temp*np.sign(x_list[1]))
            # matrix_diffusion[0,0]   -= 0.5*dt/dx *(mu_temp + param_mu_x_temp*x_list[0] + param_mu_t_temp*t_list_temp[i_t] + param_B_t_temp*np.sign(x_list[0]))
            # matrix_diffusion[-1,-2] -= 0.5*dt/dx *(mu_temp + param_mu_x_temp*x_list[-2] + param_mu_t_temp*t_list_temp[i_t] + param_B_t_temp*np.sign(x_list[-2]))
            # matrix_diffusion[-1,-1] += 0.5*dt/dx *(mu_temp + param_mu_x_temp*x_list[-1] + param_mu_t_temp*t_list_temp[i_t] + param_B_t_temp*np.sign(x_list[-1]))
            # matrix_diffusion[0,0]   -= 3.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[0] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            # matrix_diffusion[-1,-1] -= 3.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[-1] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            # matrix_diffusion[0,1]   += 3.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[1] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            # matrix_diffusion[-1,-2] += 3.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[-2] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            # matrix_diffusion[0,2]   -= 1.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[2] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)
            # matrix_diffusion[-1,-3] -= 1.*(0.5*(sigma_0_temp+ param_sigma_x_temp*x_list[-3] + param_sigma_t_temp*t_list_temp[i_t])**2*dt/dx**2)

            t1 = np.sum(matrix_diffusion)
            ...
            if bound_shift !=0:                                                                                         ## Second effect of Collapsing Bounds: Collapsing Center: Positive and Negative states are closer to each other over time.
                ...
                col_shift_list_inner_upper = range(center_matrix_ind-col_shift_inner, center_matrix_ind+col_shift_inner-1)                # Upper Off-diagonal, inner (for linear approximation)
                col_shift_list_inner_lower = range(center_matrix_ind-col_shift_inner+1, center_matrix_ind+col_shift_inner)                # Lower Off-diagonal, inner (for linear approximation)
                col_shift_list_outer_upper = range(center_matrix_ind-col_shift_outer, center_matrix_ind+col_shift_outer-1)                # Upper Off-diagonal, outer (for linear approximation)
                col_shift_list_outer_lower = range(center_matrix_ind-col_shift_outer+1, center_matrix_ind+col_shift_outer)                # Lower Off-diagonal, outer (for linear approximation)
                ...
                if col_shift_list_inner_upper !=[]:
                    matrix_diffusion[col_shift_list_inner_upper, center_matrix_ind+1*0+col_shift_inner] +=  weight_inner*np.diag(matrix_diffusion, 1)[col_shift_list_inner_upper]
                    matrix_diffusion[np.add(col_shift_list_inner_lower,1).astype(int), center_matrix_ind-1*0-col_shift_inner] +=  weight_inner*np.diag(matrix_diffusion,-1)[col_shift_list_inner_lower]
                if col_shift_list_outer_upper !=[]:
                    matrix_diffusion[col_shift_list_outer_upper, center_matrix_ind+1*0+col_shift_outer] +=  weight_outer*np.diag(matrix_diffusion, 1)[col_shift_list_outer_upper]
                    matrix_diffusion[np.add(col_shift_list_outer_lower,1).astype(int), center_matrix_ind-1*0-col_shift_outer] +=  weight_outer*np.diag(matrix_diffusion,-1)[col_shift_list_outer_lower]
            ...
            # print t1 - np.sum(matrix_diffusion)



            pdf_list[:,i_t+1] = np.linalg.solve(matrix_diffusion, pdf_list[:,i_t])                                      # Implicit method. Is np.linalg.solve_banded useful?
            # pdf_list[:,i_t+1]/= np.sum(pdf_list[:, i_t+1])
        else:
            break       #break if the remaining densities are too small....
        pdf_scale_ratio = np.sum(pdf_list[:, i_t+1]) / np.sum(pdf_list[:, i_t])
        # print (1 - np.sum(Prob_list_err) - np.sum(Prob_list_corr))/np.sum(pdf_list[:, i_t+1])
        pdf_list[:, i_t+1] *= (1 - np.sum(Prob_list_err) - np.sum(Prob_list_corr))/np.sum(pdf_list[:, i_t+1])
        Prob_list_corr[i_t+1] = pdf_list[-1,i_t+1]*pdf_scale_ratio
        Prob_list_err[i_t+1]  = pdf_list[ 0,i_t+1]*pdf_scale_ratio
        pdf_list[ 0,i_t+1] = 0.
        pdf_list[-1,i_t+1] = 0.
        ...
        #     # Prob_list_corr[i_t+1] = pdf_list[-1,i_t+1]*( (mu_temp+param_mu_x_temp*x_list[-1] + param_B_t_temp)/dx/2. + sigma_0_temp**2/2./dx**2)*dt
        #     # Prob_list_err[i_t+1]  = pdf_list[ 0,i_t+1]*(-(mu_temp+param_mu_x_temp*x_list[0]  - param_B_t_temp)/dx/2. + sigma_0_temp**2/2./dx**2)*dt
        #     Prob_list_corr[i_t+1] = pdf_list[-1,i_t+1] - pdf_list[-1,i_t]
        #     Prob_list_err[i_t+1]  = pdf_list[ 0,i_t+1] - pdf_list[ 0,i_t]
        ...
        #     print((pdf_list[0, i_t+1]))
        #     print((pdf_list[-1, i_t+1]))
