    ...
    for i_t in range(len(t_list_temp)-1):
        ...
        if sum(pdf_list[:, i_t])>0.00001:                                                                               # For efficiency only do diffusion if there's at least some densities remaining in the channel.
            ...
            if bound_shift !=0:                                                                                         ## Second effect of Collapsing Bounds: Collapsing Center: Positive and Negative states are closer to each other over time.
                ...
                weight_outer    = 0.*(bound_shift - col_shift_inner*dx)/dx                                                 # The weight of the upper bound matrix, approximated linearly.
                ...
                col_shift_list_inner_upper = range(center_matrix_ind-col_shift_inner, center_matrix_ind-1)                # Upper Off-diagonal, inner (for linear approximation)
                col_shift_list_inner_lower = range(center_matrix_ind+1, center_matrix_ind+col_shift_inner)                # Lower Off-diagonal, inner (for linear approximation)
                col_shift_list_outer_upper = range(center_matrix_ind-col_shift_outer, center_matrix_ind-1)                # Upper Off-diagonal, outer (for linear approximation)
                col_shift_list_outer_lower = range(center_matrix_ind+1, center_matrix_ind+col_shift_outer)                # Lower Off-diagonal, outer (for linear approximation)
                ...
                if col_shift_list_inner_upper !=[]:
                    ...
                    matrix_diffusion[col_shift_list_inner_upper, center_matrix_ind] +=  weight_inner*np.diag(matrix_diffusion, 1)[col_shift_list_inner_upper]
                    matrix_diffusion[np.add(col_shift_list_inner_lower,1).astype(int), center_matrix_ind] +=  weight_inner*np.diag(matrix_diffusion,-1)[col_shift_list_inner_lower]
                if col_shift_list_outer_upper !=[]:
                    ...
                    matrix_diffusion[col_shift_list_outer_upper, center_matrix_ind] +=  weight_outer*np.diag(matrix_diffusion, 1)[col_shift_list_outer_upper]
                    matrix_diffusion[np.add(col_shift_list_outer_lower,1).astype(int), center_matrix_ind] +=  weight_outer*np.diag(matrix_diffusion,-1)[col_shift_list_outer_lower]
